...
export async function validateColorBlindness(input) {
    const colors = input.colors.map(c => ({
        input: c,
        color: parseColor(c),
    }));
    // Determine which CVD types to simulate
    const cvdTypes = input.cvdType
        ? [input.cvdType]
    ...
    // Overall assessment
    const hasAnyConfusable = Object.values(results).some((r) => r.hasConfusablePairs);
    return {
...
function getPerceptualChangeDescription(deltaE) {
    if (deltaE < 1)
        return 'Imperceptible change';
    if (deltaE < 2)
        return 'Barely perceptible change';
    if (deltaE < 5)
...
//# sourceMappingURL=validateColorBlindness.js.map