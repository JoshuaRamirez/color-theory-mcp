...
export async function generateScale(input) {
    const baseColor = parseColor(input.baseColor);
    const scale = paletteService.generateScale(baseColor, input.steps ? { steps: input.steps } : undefined);
    ...
    const cssVariables = scaleColors.map(s => `--color-${s.step}: ${s.hex};`).join('\n');
    // Generate Tailwind config
    const tailwindConfig = Object.fromEntries(scaleColors.map(s => [s.step.toString(), s.hex]));
    return {
        baseColor: {
...
//# sourceMappingURL=generateScale.js.map