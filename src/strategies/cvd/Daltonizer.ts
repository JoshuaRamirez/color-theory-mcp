 ...
 */
function getErrorCategory(cvdType: CVDType): 'protan' | 'deutan' | 'tritan' {
...
export function daltonize(
  color: Color,
  cvdType: CVDType,
  options?: { severity?: number; strength?: number }
): DaltonizeResult {
  ...
  const [r, g, b] = srgb.components as [number, number, number];
  ...
  const [sr, sg, sb] = conversionService.convert(simulated, 'srgb').components as [
    number,
    number,
    number,
  ];
  ...
  const correctedLinear: [number, number, number] = [
 ...
 */
export function daltonizePalette(
  colors: Color[],
  cvdType: CVDType,
  options?: { severity?: number; strength?: number }
): DaltonizeResult[] {
