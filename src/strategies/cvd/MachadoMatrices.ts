 ...
 */
export const DEUTERANOPIA_MATRICES: Record<number, Matrix3x3> = {
  0.0: Matrix3x3.create([
    [1.000000, 0.000000, 0.000000],
    [0.000000, 1.000000, 0.000000],
    [0.000000, 0.000000, 1.000000],
 ...
 */
export const TRITANOPIA_MATRICES: Record<number, Matrix3x3> = {
  0.0: Matrix3x3.create([
    [1.000000, 0.000000, 0.000000],
    [0.000000, 1.000000, 0.000000],
    [0.000000, 0.000000, 1.000000],
 ...
 */
export function getMatrixForSeverity(
  matrices: Record<number, Matrix3x3>,
  severity: number
): Matrix3x3 {
  const clampedSeverity = Math.max(0, Math.min(1, severity));

  // Check for exact match
  const rounded = Math.round(clampedSeverity * 10) / 10;
  if (matrices[rounded]) {
    return matrices[rounded]!;
  }

  // Interpolate between two matrices
  const low = Math.floor(clampedSeverity * 10) / 10;
  const high = Math.ceil(clampedSeverity * 10) / 10;
  const t = (clampedSeverity - low) / 0.1;

  const lowMatrix = matrices[low] ?? matrices[0]!;
  const highMatrix = matrices[high] ?? matrices[1.0]!;

  // Linear interpolation of matrix values
  const interpolated: number[][] = [];
  for (let i = 0; i < 3; i++) {
    interpolated[i] = [];
    for (let j = 0; j < 3; j++) {
      interpolated[i]![j] =
        lowMatrix.values[i]![j]! * (1 - t) + highMatrix.values[i]![j]! * t;
    }
  }

  return Matrix3x3.create(interpolated);
}
