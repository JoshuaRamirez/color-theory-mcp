 ...
 */
const TEMPERATURE_BANDS: readonly { maxKelvin: number; description: string; category: string }[] = [
...
export class TemperatureService {
  ...
  colorToTemperature(color: Color): ColorTemperatureResult {
    ...
    const [X, Y, Z] = xyz.components as [number, number, number];
   ...
   */
  private describeTemperature(kelvin: number): { description: string; category: string } {
   ...
   */
  private isNearPlanckianLocus(x: number, y: number, cct: number): boolean {
   ...
   */
  private planckianLocusUV(kelvin: number): { u: number; v: number } {
    const T = kelvin;
    const T2 = T * T;
