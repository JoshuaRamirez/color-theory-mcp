...
export async function adjustColor(input: AdjustColorInput) {
  ...
  const [origL, origC, origH] = originalOklch.components as [number, number, number];
  const [adjL, adjC, adjH] = adjustedOklch.components as [number, number, number];

  const adjustments: string[] = [];
  if (input.lighten) adjustments.push(`lightened by ${Math.round(input.lighten * 100)}%`);
  if (input.darken) adjustments.push(`darkened by ${Math.round(input.darken * 100)}%`);
  if (input.saturate) adjustments.push(`saturated by ${Math.round(input.saturate * 100)}%`);
  if (input.desaturate) adjustments.push(`desaturated by ${Math.round(input.desaturate * 100)}%`);
  if (input.rotate) adjustments.push(`hue rotated by ${input.rotate}°`);
