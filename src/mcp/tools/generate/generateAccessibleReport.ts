 ...
 */
export async function generateAccessibleReport(input: GenerateAccessibleReportInput) {
  ...
  const colorResults = parsedColors.map((color, index) => {
    ...
    const colorEntry: Record<string, unknown> = {
    ...
    // CVD simulation
    if (includeCVD) {
      const cvdEntries: Record<string, { simulatedHex: string; deltaEFromOriginal: number }> = {};
