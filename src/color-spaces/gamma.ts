...
export function srgbToLinear(value: number): number {
  ...
  return Math.pow(
    (value + SRGB_GAMMA.OFFSET) / (1 + SRGB_GAMMA.OFFSET),
    SRGB_GAMMA.GAMMA
  );
}
 ...
 */
export function applyGamma(rgb: readonly [number, number, number]): [number, number, number] {
 ...
 */
export function removeGamma(rgb: readonly [number, number, number]): [number, number, number] {
