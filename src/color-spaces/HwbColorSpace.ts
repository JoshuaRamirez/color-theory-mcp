 ...
 */
export class HwbColorSpace implements IColorSpace {
  readonly type = 'hwb' as const;
  readonly componentCount = 3;
  readonly componentNames = ['Hue', 'Whiteness', 'Blackness'] as const;

  toXyzD65(color: Color): Color {
    if (color.space !== 'hwb') {
      throw new Error(`Expected hwb color, got ${color.space}`);
    }
    let [h, w, b] = color.components as [number, number, number];
  ...
  isInGamut(components: readonly number[]): boolean {
    const [h, w, b] = components as [number, number, number];
    return h >= 0 && h <= 360 && w >= 0 && w <= 1 && b >= 0 && b <= 1;
  }

  clampToGamut(components: readonly number[]): readonly number[] {
    let [h, w, b] = components as [number, number, number];
    h = ((h % 360) + 360) % 360;
